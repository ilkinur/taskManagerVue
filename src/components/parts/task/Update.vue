    ...
    <div class="row align-items-center">
      <div class="border-0 mb-4">
        <div class="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
          <h3 class="fw-bold mb-0">Update Task</h3>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async update() {
      ...
      if (validForm) {
        jwtInterceptor.post(this.$store.getters.getUrl + "/taskUpdate", this.request).then(response => {
  ...
  created() {
    this.request = user.getters.getItem;
    this.request.due_date = this.format_date(this.request.due_date)
